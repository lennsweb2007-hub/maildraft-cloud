  ...
  return (
    <div className="flex h-full flex-col p-6">
      <header className="mb-4">
        <h1 className="text-lg font-semibold text-ink-50">Versandhistorie</h1>
      ...
      {loading && !data ? (
        ...
        <div className="grid min-h-0 flex-1 gap-4 lg:grid-cols-[22rem_1fr]">
          ...
          <div className="card flex min-h-0 flex-col">
              ...
              {data.items.map((entry) => (
                <button
                  ...
                  className={`flex w-full gap-3 p-3.5 text-left transition-colors ${
                    selected?.id === entry.id ? 'bg-brand-500/10' : 'hover:bg-ink-800/50'
                  }`}
                >
                  <div
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="truncate text-sm font-medium text-ink-100">{entry.to_email}</p>
                    <p className="truncate text-xs text-ink-400">{entry.subject}</p>
            ...
            {(hasMore || offset > 0) && (
              <div className="flex items-center justify-between gap-2 border-t border-ink-800 p-2.5">
                <button
                  type="button"
  ...
  return (
    <div className="card flex min-h-0 flex-col">
      <header className="border-b border-ink-800 px-5 py-4">
        <h2 className="text-base font-semibold text-ink-50">{entry.subject}</h2>
        <div className="mt-1.5 flex flex-wrap items-center gap-2.5 text-xs text-ink-400">
      ...
      <div className="grid min-h-0 flex-1 divide-y divide-ink-800 overflow-y-auto md:grid-cols-2 md:divide-x md:divide-y-0">
